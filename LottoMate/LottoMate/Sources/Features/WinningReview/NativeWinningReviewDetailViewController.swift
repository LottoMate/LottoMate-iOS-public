...
/**
 * FlexLayout & PinLayout을 사용하여 구현 시 처음 당첨 후기 상세 화면이 나타날 때
 * 서버 데이터가 보여지지 않는 문제가 있어 임시로 auto layout 을 사용해서 구현 함
 */
class NativeWinningReviewDetailViewController: UIViewController {
    ...
    private lazy var horizontalReviewCards: WinningReviewListView = {
        let view = WinningReviewListView(cardSize: .small, showDotIndicator: false, useSharedReactor: false)
        view.translatesAutoresizingMaskIntoConstraints = false
        return view
    }()
    ...
    private func setupConstraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            // Custom Navigation Bar (on top of scrollView, extends to status bar)
            customNavBar.topAnchor.constraint(equalTo: view.topAnchor),
            customNavBar.leadingAnchor.constraint(equalTo: view.leadingAnchor),
            customNavBar.trailingAnchor.constraint(equalTo: view.trailingAnchor),
            customNavBar.bottomAnchor.constraint(equalTo: safeArea.topAnchor, constant: navBarHeight),
            
            // Back Button (positioned relative to safe area)
            ...
            // Nav Title Label (positioned relative to safe area)
...
//            horizontalReviewCards.bottomAnchor.constraint(equalTo: contentView.bottomAnchor, constant: -18),
            ...
            bannerContainer.topAnchor.constraint(equalTo: horizontalReviewCards.bottomAnchor, constant: 40),
            ...
            bannerContainer.bottomAnchor.constraint(equalTo: contentView.bottomAnchor, constant: -58),
        ])
        
        // Set initial top constraint for contentStackView (will be updated dynamically based on image availability)
        contentStackViewTopConstraint = contentStackView.topAnchor.constraint(equalTo: imageContainerView.bottomAnchor, constant: 28) // 패딩이 생기지 않아 확인 필요
    ...
    @objc private func originalReviewTapped() {
        ...
        let urlString = "https://dhlottery.co.kr/gameResult.do?method=highWinView&txtNo=\(reviewHref)"
        ...
        apiService.fetchWinningReviewDetail(reviewNo: reviewNo)
            ...
            .subscribe(
                onNext: { [weak self] response in
                    ...
                    // 상세 데이터 로드 완료 후 하단 후기 목록 로드
                    self?.loadHorizontalReviews()
                },
    ...
    private func loadHorizontalReviews() {
        print("📡 Fetching horizontal reviews based on current reviewNo: \(reviewNo)")
        
        // 현재 reviewNo를 기준으로 그보다 작은 번호 중 최신 5개 생성
        // 예: 현재 번호가 17이면 -> [16, 15, 14, 13, 12]
        var reviewNos: [Int] = []
        var offset = 1
        
        while reviewNos.count < 5 && (reviewNo - offset) > 0 {
            reviewNos.append(reviewNo - offset)
            offset += 1
        }
        
        print("📋 Generated review numbers for horizontal list: \(reviewNos)")
        
        guard !reviewNos.isEmpty else {
            print("⚠️ No review numbers to fetch (current reviewNo is too small)")
            horizontalReviewCards.updateWinningReviews(with: [])
            return
        }
        
        apiService.fetchWinningReviewList(reviewNos: reviewNos)
            .observe(on: MainScheduler.instance)
            .subscribe(
                onNext: { [weak self] reviews in
                    // reviewNo 기준 내림차순 정렬 (최신순)
                    let sortedReviews = reviews.sorted { $0.reviewNo > $1.reviewNo }
                    print("✅ Successfully loaded \(sortedReviews.count) horizontal reviews: \(sortedReviews.map { $0.reviewNo })")
                    self?.horizontalReviewCards.updateWinningReviews(with: sortedReviews)
                },
                onError: { [weak self] error in
                    print("❌ Error loading horizontal reviews: \(error)")
                    // 에러 발생 시에도 UI는 정상 표시 (후기 목록만 비어있음)
                    self?.horizontalReviewCards.updateWinningReviews(with: [])
                }
            )
            .disposed(by: disposeBag)
    }
...
extension NativeWinningReviewDetailViewController: BannerNavigationDelegate {
    ...
    private func showWinningReviewDetail(reviewNo: Int) {
        print("🎯 NativeWinningReviewDetailViewController: Showing review detail for reviewNo: \(reviewNo)")
        
        let detailVC = NativeWinningReviewDetailViewController(reviewNo: reviewNo)
        
        if let navigationController = self.navigationController {
            print("✅ NativeWinningReviewDetailViewController: Pushing detail view via navigation controller")
            navigationController.pushViewController(detailVC, animated: true)
        } else {
            print("❌ NativeWinningReviewDetailViewController: No navigation controller found")
        }
    }
}

extension NativeWinningReviewDetailViewController: WinningReviewListViewDelegate {
